...
class Classification(BaseModel):
    message_type: Literal["photo", "food_question", "dialog"] = Field(
        description = "Тип сообщения: photo - ссылка на фото; food_question - вопрос по питанию; dialog - простой диалог"
    )
...
classification_prompt = PromptTemplate(template = """Определи тип сообщения пользователя:
    ...
    FOOD_QUESTION - вопрос, связанный только с питанием или едой
    DIALOG - сообщение, которое не связано с едой или питанием
...
def classify_message(state: SystemState) -> dict:
    ...
    try:
        ...
        message_type = classification_result.get("message_type", "dialog")
        confidence = classification_result.get("confidence", 0.0)
    except Exception as e:
        print(f"Ошибка классификации: {e}. Используем 'dialog' по умолчанию.")
        message_type = "dialog"
        confidence = 0.0
...
@graph.add_node
async def dialog(state: SystemState):
    # Извлекаем последнее сообщение пользователя, но можно передать все
    # Для простоты передадим всю историю, но лучше последнее сообщение или всю историю, как нужно
    # В LangChain можно просто вызвать llm с сообщениями
    messages = state.get("messages", [])
    if not messages:
        return {"messages": [HumanMessage(content="Нет сообщений для ответа.")]}
    response = await llm.ainvoke(messages)
    # response — это AIMessage, извлекаем content
    answer_content = response.content
    print(f"ИИ: {answer_content}")
    return {
        ...
        async with queue.iterator() as queue_iter:
            ...
            async for message in queue_iter:
                async with message.process(requeue=False):
                    ...
                    # Извлекаем детекции (если есть)
                    sizes = result.get("size_info", [])
                    calories = result.get("result_calories", "")
                    ...
                    # Отправляем ответ в response_queue
